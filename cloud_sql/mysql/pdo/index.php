...
$app->get('/', function ($request, $response) {
    return $this->get('view')->render($response, 'template.twig', [
        ...
        'tabCount' => $this->get('votes')->getCountByTeam('TABS'),
        'spaceCount' => $this->get('votes')->getCountByTeam('SPACES'),
    ]);
});
...
$app->post('/', function ($request, $response) {
    ...
    if (in_array($formData['team'], ['SPACES', 'TABS'])) {
        $message = $this->get('votes')->insertVote($formData['team'])
            ? 'Vote cast for ' . $formData['team']
            : 'An error occurred';
    }
